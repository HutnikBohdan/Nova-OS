...
pub fn hkdf_extract(salt: &[u8], input_key: &[u8]) -> [u8; 32] {
    if salt.is_empty() {
        hmac_sha256(&[0u8; 32], input_key)
    } else {
        hmac_sha256(salt, input_key)
    }
}
...
pub fn hkdf_expand(prk: &[u8], info: &[u8], output: &mut [u8]) -> Result<(), CryptoError> {
    ...
    while written < output.len() {
        let mut buffer = [0u8; 32 + 255 + 1];
        if info.len() > 255 {
            return Err(CryptoError::OutputTooLarge);
        }
        buffer[..previous_len].copy_from_slice(&previous[..previous_len]);
        buffer[previous_len..previous_len + info.len()].copy_from_slice(info);
        buffer[previous_len + info.len()] = counter;
        previous = hmac_sha256(prk, &buffer[..previous_len + info.len() + 1]);
        previous_len = 32;
